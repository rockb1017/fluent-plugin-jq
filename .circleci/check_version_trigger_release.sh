...
if [ $VERSION_COMMIT = $LATEST_COMMIT ];
    then
        ...
        git push https://$RELEASE_GITHUB_USER:$RELEASE_GITHUB_PASS@github.com/rockb1017/fluent-plugin-jq.git release/$VERSION
        git checkout master
        git merge --no-edit release/$VERSION
        git push https://$RELEASE_GITHUB_USER:$RELEASE_GITHUB_PASS@github.com/rockb1017/fluent-plugin-jq.git master
fi
